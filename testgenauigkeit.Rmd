...
question_text("Wie wurde die Richtig-Positiv-Rate, bzw. Sensitivität von 91.4% aus der Konfusionsmatrix im Beipackzettel berechnet?",
  answer_fn(function (input) {
      expected <- "139/152*100"
      input <- gsub("\\s+", "", input)  # Leerzeichen im Text ignorieren
      
      if (input == gsub("\\*100$", "", expected)) {
        incorrect('Fast richtig. Multiplizieren Sie das Ergebnis noch mit 100 um einen Prozentwert zu bekommen.')
      } else if (!grepl("^\\d+/\\d+\\*100$", input)) {
        incorrect('Geben Sie eine Prozentrechnung der Form "a/b*100" ein.')
      } else {
        mark_as(input == expected)
      }
  }),
  allow_retry = TRUE,
  trim = TRUE,
