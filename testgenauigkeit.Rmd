...
library(learnrextra)   # Erweiterungen für learnr
...
incorrect_too_long <- "Deine Eingabe ist leider zu lang."
incorrect_comma <- "Deine Eingabe enthält ein Komma. Bitte benutze wie in R den Punkt als Dezimaltrennzeichen, statt das Komma."
incorrect_invalid_symbols <- "Deine Eingabe enthält ungültige Symbole oder Funktionen. Nur folgende Symbole und Funktionen sind möglich: "
incorrect_cannot_evaluate <- "Deine Eingabe ist leider keine gültige Zahl bzw. kein gültiger mathematischer Ausdruck."
incorrect_out_of_range_probability <- "Gefragt ist eine Wahrscheinlichkeit, daher muss das Ergebnis zwischen %d und %d sein, deine Antwort ist aber %f."
placeholder <- "Mathematischer Ausdruck oder Zahl"

question_mathexpression_probability_default <- function(text, expected_result) {
    question_mathexpression_probability(
        paste0(text, "\n\nGeben Sie entweder einen mathematischen Ausdruck wie `(20+11)/10` ein oder eine Zahl wie `3.1` (Achtung, das Dezimalzeichen ist der Punkt)."),
        expected_result,
        handle_comma_in_input = incorrect_comma,
        correct = correct_msg,
        incorrect = incorrect_msg,
        incorrect_too_long = incorrect_too_long,
        incorrect_invalid_symbols = incorrect_invalid_symbols,
        incorrect_cannot_evaluate = incorrect_cannot_evaluate,
        incorrect_out_of_range = incorrect_out_of_range_probability,
...
options("tutorial.max.forked.procs" = 100)
...
![Der Beipackzettel eines handelsüblichen Covid-19-Schnelltests](images/test-performance-clungene2edit.png){height=275px}
...
```{r clinicalstudy2}
question_numeric("Bei wie vielen Personen in der Studie hat der Test den Gesundheitszustand (krank/gesund) richtig erkannt?",
...
```{r calc_sens}
answer_fn_fractions_percentage <- function(expected_numerator, expected_denominator, correct_msg, incorrect_msg) {
    function (input) {
      input <- gsub("[\\s+%]", "", input)  # Leerzeichen und Prozentzeichen im Text ignorieren

      m <- regexec("(\\d+)\\/(\\d+)", input)
      m <- regmatches(input, m)

      if (length(m) == 0 || length(m[[1]]) != 3) {
          return(incorrect('Geben Sie eine Prozentrechnung der Form "a/b*100" ein.'))
      }
      
      inp_a <- m[[1]][2]
      inp_b <- m[[1]][3]
      perc <- startsWith(input, "100*") || endsWith(input, "*100")

      if (inp_a == expected_numerator && inp_b == expected_denominator) {
          if (!perc) {
              return(incorrect('Fast richtig. Multiplizieren Sie die ermittelte Wahrscheinlichkeit mit 100 um einen Prozentwert zu bekommen.'))
          }
...
question_text_custom_answer_fn(
    "Wie wurde die Richtig-Positiv-Rate, bzw. Sensitivität von 91.4% aus der Konfusionsmatrix 
     im Beipackzettel berechnet?",
    answer_fn_fractions_percentage(139, 152, correct_msg = correct_msg, incorrect_msg = incorrect_msg),
    allow_retry = TRUE,
    trim = TRUE,
...
```{r calc_spec}
question_text_custom_answer_fn(
    "Wie wird die Richtig-Negativ-Rate, bzw. Spezifität von 99.4% aus der Konfusionsmatrix im Beipackzettel berechnet?",
    answer_fn_fractions_percentage(462, 465, correct_msg = correct_msg, incorrect_msg = incorrect_msg),
    allow_retry = TRUE,
    trim = TRUE,
    placeholder = 'Geben Sie eine Prozentrechnung der Form "a/b*100" ein.'
)
...
Warum beantworten Testgenauigkeit, Sensitivität und Spezifität nicht unsere eigentliche Fragestellung<span class="survey_zusammenfassung">, die wir in der Zusammenfassung rechts oben sehen</span>? Das werden wir uns auf der folgenden Seite anschauen.
...
```{r P_K}
question_mathexpression_probability_default(
...
```{r P_K_T_vergleich}
question("Wie unterscheidet sich der oben berechnete positive Vorhersagewert $P(K|T)$ für eine Prävalenz von
          $P(K)=0.25$ zum Wert von $P(K|T)$ für die Prävalenz $P(K)=0.002$ aus der Beispielrechnung?",
  answer("$P(K|T)$ ist niedriger für eine Prävalenz von $P(K)=0.25$, als für eine  Prävalenz von $P(K)=0.002$."),
  answer("$P(K|T)$ für eine Prävalenz von $P(K)=0.25$ ist in etwa gleich $P(K|T)$ für $P(K)=0.002$."),
  answer("$P(K|T)$ ist höher für eine Prävalenz von $P(K)=0.25$, als für eine Prävalenz von $P(K)=0.002$.",
         correct = TRUE),
  allow_retry = TRUE,
  correct = correct_msg,
  incorrect = incorrect_msg
...
anzuwenden.",
  0.994 * (1-0.002) / (0.994 * (1-0.002) + (1-0.914) * 0.002)
)
```
...
survey_likert(
    items = c(
        ...
        "praxisbeispiel" = "Medizinische Tests als Praxisbeispiel für den Satz von Bayes finde ich ...",
        "verwendung_uebung" = "Solche Lernanwendungen sollten in der Übung ...",
        "verwendung_zuhause" = "Ich würde solche Lernanwendungen zum Selbstlernen zu Hause ...",
        "bedienung" = "Das Layout und die Bedienung der Lernanwendung sind ...",
        "interakt_grafik" = "Die interaktive Grafik fand ich ...",
        "mehrere_tabs" = "Haben oder hatten Sie mehrere Fenster oder Tabs dieser Seite geöffnet?",
        "zusammenfassung" = "Ich habe die Zusammenfassungsleiste auf der rechten Seite ...",
        "umfang" = "Vom Umfang her ist diese Lernanwendung ...",
        "zufriedenheit" = "Die Lernanwendung finde ich insgesamt ..."
    ),
    levels = list(
        paste0("... ", c("sehr schwer", "eher schwer", "genau richtig", "eher leicht", "zu leicht"), "."),
        paste0("... ", c("gar nicht interessant", "eher wenig interessant", "mittelmäßig interessant", "eher interessant", "sehr interessant"), "."),
        paste0("... ", c("nie verwendet werden", "eher selten verwendet werden", "ab und an verwendet werden", "eher oft verwendet werden", "immer verwendet werden"), "."),
        paste0("... ", c("nie verwenden", "eher selten verwenden", "ab und an verwenden", "eher oft verwenden", "ständig verwenden"), "."),
        paste0("... ", c("sehr schwer verständlich", "eher schwer verständlich", "mittelmäßig verständlich", "eher leicht verständlich", "sehr leicht verständlich"), "."),
        paste0("... ", c("gar nicht hilfreich", "eher wenig hilfreich", "mittelmäßig hilfreich", "eher hilfreich", "sehr hilfreich"), "."),
        paste0(c("Ja", "Nein", "Weiß nicht / verstehe die Frage nicht"), "."),
        paste0("... ", c("gar nicht genutzt", "selten genutzt", "ab und an genutzt", "eher oft genutzt", "ständig genutzt"), "."),
        paste0("... ", c("viel zu kurz", "etwas zu kurz", "genau richtig", "etwas zu lang", "viel zu lang"), "."),
        paste0("... ", c("sehr schlecht", "eher schlecht", "mittelmäßig", "eher gut", "sehr gut"), ".")
    ),
    caption = "Umfrage", message = "Danke."
)
```
...
survey(
    list(
        list(
            text = "Was würden Sie an der Lernanwendung verbessern?",
            label = "survey-kommentar",
            type = "learnr_text"
        )
    ),
    caption = "",
...
<ul>
    ...
    <li class="trackingdata-ip">IP-Adresse</li>
    <li class="trackingdata-user_agent">Browsersoftware</li>
    <li class="trackingdata-device_info">Gerätetyp und Fenstergröße des Browsers</li>
    <li class="trackingdata-visibility">Anzeigestatus des Browserfensters</li>
    <li class="trackingdata-mouse">Mausbewegungen</li>
    <li class="trackingdata-clicks">Klicks bzw. Taps</li>
    <li class="trackingdata-scrolling">Scrollbewegungen</li>
    <li class="trackingdata-inputs">Änderungen an Bedienelementen (für interaktive Grafiken, etc.)</li>
    <li class="trackingdata-attribute_changes">Änderungen an HTML Elementen</li>
    <li class="trackingdata-chapters">Informationen über Seitenwechsel</li>
    <li class="trackingdata-summary">Anzeige und Aktualisierung der Zusammenfassungsleiste</li>
    <li class="trackingdata-exercise_hint">Informationen beim Ausfüllen der Code-Aufgaben (Anzeige des Hinweises)</li>
    <li class="trackingdata-exercise_submitted">Informationen beim Ausfüllen der Code-Aufgaben (Abgabe)</li>
    <li class="trackingdata-exercise_result">Informationen beim Ausfüllen der Code-Aufgaben (Resultat)</li>
    <li class="trackingdata-question_submission">Informationen beim Ausfüllen der Quizze (gegebene Antworten, Resultat)</li>
    <li class="trackingdata-video_progress">Informationen über das Abspielen von Videos</li>
</ul>
