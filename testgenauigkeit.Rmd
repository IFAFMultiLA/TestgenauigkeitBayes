...
(Achtung, das Dezimalzeichen ist der Punkt).",
  answer_fn(function(input) {
    ...
    if (!is.null(result)) {
        if (identical(round(result, 2), round(expected_result, 2))) {
            correct(sprintf("Das Ergebnis ist %.3f.", result))
        } else if (result < 0 || result > 1) {
            incorrect(sprintf("Beachten Sie, dass eine Wahrscheinlichkeit gefragt ist. Eine
                              Wahrscheinlichkeit muss einen Wert zwischen 0 und 1 annehmen, 
                              Sie haben aber %f eingeben.", result))
        } else {
            incorrect(NULL)
        }
    } else {
