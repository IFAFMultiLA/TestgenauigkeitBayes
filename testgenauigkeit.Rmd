...
```{r setup_js, echo=FALSE}
htmltools::tagList(
  htmltools::tags$script(src = "www/tutorial.js")
)
```
...
```{r, warning=FALSE}
#ppv_by_param$sens_spec_lbl <- sprintf("Sens. %.3f%%, Spez. %.3f%%", ppv_by_param$sens * 100, ppv_by_param$spec * 100)
...
question("Warum spielt die Sensitivität bei niedriger Prävalenz keine so starke Rolle für den positiven Vorhersagewert?",
  answer("Da bei niedriger Prävalenz die Wahrscheinlichkeit sehr gering ist, erkrankt zu sein, ganz unabhängig von der Sensitivität.", correct = TRUE),
...
grade_this({
    ...
    if (all(dplyr::near(.result, nvw_param))) {
        pass("Korrekt!")
    }
    fail()
})
